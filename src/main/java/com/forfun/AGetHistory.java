...
public class AGetHistory {
    ...
    public void collectData() {
        ...
        writeToFile(this.dataFile, processData);
    ...
    private void writeToFile(String filePath, ArrayList hisData){
        try {
            StringBuffer outputStr = new StringBuffer();
    ...
    // 根据历史数据计算回撤
    private ArrayList<FundHistoricData> setWithdraw(ArrayList<FundHistoricData> hisData) {
        ...
        for(int i=1; i < hisData.size(); i++) {
            ...
            * 其中r(t)就是基金投资端的实际收益率，计算公式为r(t) = U(t) / u(t-1) - 1 = [u(t)*s(t)+d(t)] / u(t-1) - 1，
            ...
            */
            if (FHSPflag) {
                ...
                if (dayTwo.JZZZL != null) {
                    fqjz = (float) Math.round( fqjz * (1+ Float.valueOf(dayTwo.JZZZL) /100) * 100000 )/100000;
                }
            } else {
    ...
    private JSONObject getContent(int pageIndex) {
        ...
        try {
            ...
            String uri = MessageFormat.format(uriPattern, fundCode, pageIndex, Constant.pageSize, startDate, endDate);
