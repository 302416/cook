		...
		try {
			...
			while ((line = br.readLine()) != null) {
				...
				if (items.length > 10) {	// 如果
					String number = items[1];	// 基金代码
					String name = items[2];		// 基金名称
					String preCheckPointDesc = items[7];
					...
					if (number.length() == 0 || number.equals("-")) {
						...
						// 计算相对于历史最大回撤
						Float withdrawPercent = (float) Math.round( aGetHist.getLatestNode().withdraw /aGetHist.getLargestWithdrawNode().withdraw * 10000 )/100;
						...
						outputBuffer.append(String.valueOf(i))
								...
								.append(Constant.splitter)
								.append(aGetHist.getLargestWithdrawNode().getPreHighDWJZ() + " -> " + aGetHist.getLargestWithdrawNode().getDWJZ())
								.append(Constant.splitter)
								.append(aGetHist.getLargestWithdrawNode().getPreHighFSRQ() + " -> " + aGetHist.getLargestWithdrawNode().getFSRQ())
								.append(Constant.splitter)
								.append(Constant.splitter)
